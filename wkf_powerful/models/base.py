...
# from odoo.addons.base.models.ir_cron import str2tuple
from pyutillib import string_utils
...
def wkf_trans_condition_expr_eval(self, lines):
    ...
    for line in lines.split('\n'):
        ...
        if line == 'True':
            ...
            # env = Environment(self.env, self)
            env = api.Environment(self.env, self)
...
def default_get_new(self, fields_list):
    ...
    if 'x_wkf_state' in fields_list:
        res.update(
            {'x_wkf_state': self.env['wkf.base'].get_default_state(self._name)})
    return res
...
def wkf_action(self, message=''):
    ...
    if not condition_ok:
        if trans.auto:
            ...
            raise Warning(
                u'Th condition is not allow to trans, Pleas contract with Administrator')
    ...
    if can_trans:
        ...
        if trans.is_backward:
            ...
            if action:
                ...
                # args = str2tuple(arg)
                # args = string_utils.str2tuple(args)
...
def wkf_button_show_log(self):
    return {
        ...
        'view_id': False,
        'target': 'new',
        'domain': [('res_id', '=', self[0].id,), '|', ('active', '=', True), ('active', '=', False)],
...
def wkf_button_reset(self):
    logs = self.env['log.wkf.trans'].search(
        [('res_id', '=', self[0].id), ('model', '=', self._name)])
